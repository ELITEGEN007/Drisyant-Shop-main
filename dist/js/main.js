...
function initHeroTitleImageHover() {
    ...
    titleImages.forEach(img => {
        ...
        img.addEventListener('animationend', (e) => {
            if (e.animationName && e.animationName.includes('shrink')) {
                // Remove animation to allow hover transition to work cleanly
                img.style.animation = 'none';
                if (img.id === 'selfie') {
                    const size = isMobileDevice() ? '50px' : '7.2vw';
                    img.style.width = size;
                    img.style.height = size;
                    img.style.marginRight = '.6vw';
                    img.style.opacity = '1';
                    img.style.transform = isMobileDevice() ? 'translateY(-4px)' : 'translateY(-0.8vw)';
                } else {
                    img.style.width = isMobileDevice() ? '45px' : '7.5vw';
                    img.style.marginRight = '.8vw';
                }
            }
        });
    });
...
function initWorkLaptopSequence() {
    ...
    const framePad = 5;
    const videoSrc = 'assets/video-featured.mp4';
...
function initWorkPageSlider() {
    ...
    mediaElements.forEach(media => {
        if (media.tagName === 'VIDEO') {
            ...
            media.querySelectorAll('video').forEach(v => v.load());
        }
    });
...
function initThumbnailMarqueeParallax() {
    const container = document.getElementById('thumbMarquee');
    if (!container) return;

    const rowLeft = container.querySelector('.thumb-marquee-track--left');
    const rowRight = container.querySelector('.thumb-marquee-track--right');
    ...
    const updateParallax = () => {
        ...
        if (rect.bottom > -100 && rect.top < viewH + 100) {
            ...
            const shift = (scrollProgress - 0.5) * 160;
            rowLeft.style.setProperty('--scroll-shift', `${-shift}px`);
            rowRight.style.setProperty('--scroll-shift', `${shift}px`);
        }
        ticking = false;
    };
...
document.addEventListener('DOMContentLoaded', () => {
    initThumbnailMarqueeParallax();
});

if (document.querySelector('#thumbMarquee')) {
    initThumbnailMarqueeParallax();
}
